...
class Catridge {
  public:
	Catridge(const std::string path) {
		...
		if(!file.read(m_rawData.data(), size)) { throw std::runtime_error("Failed to read file.\n"); }
	}
	...
	std::string title() const { return std::string(m_rawData.data() + CATRIDGE_TITLE_OFFSET, CATRIDGE_TITLE_LEN); }
	uint8_t type() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_TYPE_OFFSET)); }
	...
	uint16_t newLicenseeCode() const {
		uint8_t high = static_cast<uint8_t>(m_rawData.at(CATRIDGE_NEW_LICENSEE_CODE_OFFSET));
		uint8_t low = static_cast<uint8_t>(m_rawData.at(CATRIDGE_NEW_LICENSEE_CODE_OFFSET + 1));

		return (static_cast<uint16_t>(high) << 8) | static_cast<uint16_t>(low);
	}
	uint8_t oldLicenseeCode() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_OLD_LICENSEE_CODE_OFFSET)); }

	uint8_t flagSGB() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_SGB_FLAG_OFFSET)); }
	uint8_t flagCGB() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_CGB_FLAG_OFFSET)); }

	uint8_t romSize() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_RAM_SIZE_OFFSET)); }
	uint8_t ramSize() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_RAM_SIZE_OFFSET)); }


	std::vector<uint8_t> manufracturerCode() const {
		return std::vector<uint8_t>(m_rawData.begin() + CATRIDGE_MANUFRACTURER_CODE_OFFSET,
									m_rawData.begin() + CATRIDGE_MANUFRACTURER_CODE_OFFSET +
										CATRIDGE_MANUFRACTURER_CODE_LEN);
	}

	uint8_t romVersion() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_ROM_VERSION_OFFSET)); }
	uint8_t destinationCode() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_DESTINATION_CODE_OFFSET)); }

	uint8_t headerChecksum() const { return static_cast<uint8_t>(m_rawData.at(CATRIDGE_HEADER_CHECKSUM_OFFSET)); }

	uint8_t calculateHeaderChecksum(void) const {
		uint8_t checksum = 0;
		for(uint16_t address = 0x0134; address <= 0x014C; address++) {
			checksum = checksum - m_rawData[address] - 1;
		}
